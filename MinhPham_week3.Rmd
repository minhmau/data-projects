...
boxplot(dat$faminc_new,
     main="Minh's Boxplot of Family Income",
     xlab = "Family Income")
...
dat_1 <- dat %>% filter(gender==1 & (region == 1 | region ==2) & marstat==1 & newsint==1)
...
dat_q7 <- dat %>% mutate(tax_scale=CC18_325d + CC18_325a)
dat_q7$tax_scale <- recode(dat_q7$tax_scale, "2"=2, "3"=1,"4"=0 )
dat_q7$tax_scale[0:20]
```
...
```{r problem10}
dat %>% filter(investor==2 & (faminc_new==5 | faminc_new==6 | faminc_new==7 | faminc_new==8 | faminc_new==9 | faminc_new==10)) %>% summarise("Mean Religion Imp."=mean(pew_religimp), "Median Religion Imp." = median(pew_religimp), "Standard Dev. Religion Imp."=sd(pew_religimp))
...
```{r problem11}
thetable <- dat %>% summarise("Mean"=mean(faminc_new), "Median"=median(faminc_new), "Std. Dev."=sd(faminc_new))

kable(table(thetable), align="l")
...
qplot(x=pid7, data=dat, geom="histogram", bin=5, xlab="Seven Point Party ID", ylab = "Count")
```