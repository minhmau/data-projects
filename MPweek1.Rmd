...
cel1 <- cel1 %>% rename(Gender=female)
cel1$Gender <- recode(cel1$Gender, `0`="Male", `1`="Female")

ggplot(cel1, aes(x=dwnom1, y=all_pass, color=Gender)) + geom_point() +
...
cel1$majority <- as.factor(recode(cel1$majority, `1`="Majority", `0`="Minority"))

ggplot(cel1, aes(x=votepct, y=all_pass, color=Gender)) + geom_point() +
  scale_color_manual(values = c("orange", "green")) +
  labs(x="Vote Percentage", y="Bills Passed") +facet_wrap(~majority)
```
...
ggplot(cel1, aes(y=les, x=majority, group=majority)) + geom_boxplot() +
