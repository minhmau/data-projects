...
ggplot(map_data_combined, mapping = aes(x=long, y=lat, group=group, fill=Score))+
...
q2data <- maps::world.cities

some_contries <- filter(q2data, country.etc=="Costa Rica" | country.etc=="Panama" | country.etc=="Nicaragua" , pop > 40000)

somecountries_data_combined <- add_column(some_contries, Measurement)

world <- map_data("world")
southamerica <- filter(world, region=="Costa Rica" | region=="Panama" | region=="Nicaragua")

ggplot(data=southamerica, mapping= aes(x=long, y=lat, group=group)) + 
  geom_polygon(color="black", fill="white")+
  geom_point(data= somecountries_data_combined, aes(x=long, y=lat, group=NULL, color=Measurement, size=5))+
