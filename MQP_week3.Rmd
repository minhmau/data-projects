...
ggplotly(
ggplot(my_dat, aes(x=factor(Company), y=Output, fill=Company))+ geom_boxplot() + labs(x="Company", y="Output")
)
...
# add ids for object constancy
ggplotly(
ggplot(dat3,aes(y= Quantity, x=Category, xend=Category, frame=Time, ids=Quantity ))+
  geom_point()+
